...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработчики калькуляторов
        ...
        await query.edit_message_text(
            ...
            "`/calc_concrete длина ширина высота класс`\n\n"
            "**Пример:**\n"
            "`/calc_concrete 10 8 0.2 B25`\n\n"
            "Где:\n"
            "• Длина (м)\n"
            "• Ширина (м)\n"
            "• Высота (м)\n"
        ...
        await query.edit_message_text(
            ...
            "Введите параметры в формате:\n"
            "`/calc_reinforcement длина ширина высота диаметр шаг тип`\n\n"
            "**Пример:**\n"
            "`/calc_reinforcement 10 8 0.2 12 200 slab`\n\n"
            "Типы элементов:\n"
            "• `slab` - плита\n"
            "• `beam` - балка\n"
            "• `column` - колонна",
    ...
    elif query.data == "calc_formwork":
        await query.edit_message_text(
            "📦 **КАЛЬКУЛЯТОР ОПАЛУБКИ**\n\n"
            "Введите параметры в формате:\n"
            "`/calc_formwork площадь срок_строительства тип`\n\n"
            "**Пример:**\n"
            "`/calc_formwork 500 180 plywood`\n\n"
            "Типы опалубки:\n"
            "• `plywood` - фанера (30-40 оборотов)\n"
            "• `metal` - металл (100-150 оборотов)",
            parse_mode='Markdown'
        )

    elif query.data == "calc_electrical":
        await query.edit_message_text(
            "⚡ **КАЛЬКУЛЯТОР ЭЛЕКТРОСНАБЖЕНИЯ**\n\n"
            "Введите параметры в формате:\n"
            "`/calc_electrical кран насос сварки пушки бытовки`\n\n"
            "**Пример:**\n"
            "`/calc_electrical 1 1 5 10 10`\n\n"
            "Где количество:\n"
            "• Башенных кранов (60 кВт)\n"
            "• Бетононасосов (40 кВт)\n"
            "• Сварочных постов (7 кВт)\n"
            "• Тепловых пушек (5 кВт)\n"
            "• Бытовок (3 кВт)",
            parse_mode='Markdown'
        )

    elif query.data == "calc_water":
        await query.edit_message_text(
            "💧 **КАЛЬКУЛЯТОР ВОДОСНАБЖЕНИЯ**\n\n"
            "Введите параметры в формате:\n"
            "`/calc_water рабочие замесы`\n\n"
            "**Пример:**\n"
            "`/calc_water 50 5`\n\n"
            "Где:\n"
            "• Количество рабочих (чел)\n"
            "• Замесов бетона в день\n\n"
            "Расчёт включает:\n"
            "• Питьевую воду (25 л/чел·смену)\n"
            "• Душевые (40 л/чел·смену)\n"
            "• Мойку бетономешалки (300 л/замес)\n"
