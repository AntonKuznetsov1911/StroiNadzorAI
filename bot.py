...
def call_grok_with_retry(client, model, messages, max_tokens, temperature, tools=None):
    ...
    Args:
        tools: Список инструментов [{"type": "live_search", "sources": [{"type": "web"}, {"type": "news"}, {"type": "x"}]}, {"type": "x_search"}]
    """
...
Ответь ТОЛЬКО одним словом из списка выше:"""
        ...
        response = call_grok_with_retry(
            client,
            model="grok-3",  # Быстрая модель для классификации
            max_tokens=50,
            temperature=0.1,
        ...
        if intent_type == "simple_save" or intent_type == "simple_question":
            model = "grok-3"  # Быстрая модель для простых запросов
            max_tokens = 1000
        elif intent_type == "technical_question":
            model = "grok-3"  # Reasoning модель для технических вопросов
            max_tokens = 5000
        else:  # complex_analysis
            model = "grok-3"  # Reasoning модель для сложного анализа
            max_tokens = 8000
...
ВАЖНО: Объективность превыше всего. Если дефектов нет — так и напишите. Не ищите проблемы там, где их нет!"""
        ...
        response = await loop.run_in_executor(
            ...
            lambda: call_grok_with_retry(
                client,
                model="grok-3",  # Reasoning модель для анализа изображений
                max_tokens=6000,
                temperature=0.7,
                messages=[
                    {
...
{pdf_text}"""
                    ...
                    response = await loop.run_in_executor(
                        ...
                        lambda: call_grok_with_retry(
                            client,
                            model="grok-3",  # Reasoning модель для анализа документов
                            max_tokens=6000,
                            temperature=0.3,
                            messages=[
...
• У вас есть доступ к инструментам live_search и x_search
• Для каждого запроса старайтесь проверять актуальность через live_search
• ОБЯЗАТЕЛЬНО используйте live_search (и при необходимости x_search), если в вопросе есть:
...
В конце (если уместно) задайте 1 короткий уточняющий вопрос для продолжения диалога."""
        ...
        # 🌐 ВЕБ-ПОИСК: Теперь выполняется через Grok инструменты (live_search, x_search)
        ...
        if STREAMING_ENABLED:
            ...
            try:
                ...
                if len(first_phase_answer) >= 400 and selected_model != "grok-3":
        ...
        if IMPROVEMENTS_V3_AVAILABLE:
            try:
                ...
                related_response = await loop.run_in_executor(
                    ...
                    lambda: call_grok_with_retry(
                        client,
                        model="grok-3",  # Используем быструю модель
                        max_tokens=300,
                        temperature=0.8,
        ...
        # Веб-поиск теперь выполняется через инструменты Grok (live_search, x_search)
