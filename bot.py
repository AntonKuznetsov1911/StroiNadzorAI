...
В конце (если уместно) задайте 1 короткий уточняющий вопрос для продолжения диалога."""
        ...
        if GEMINI_AVAILABLE and should_generate_image(question):
            ...
            generating_msg = await update.message.reply_text(
                ...
                "Шаг 1/2: xAI Grok создает детальный промпт..."
            )
            ...
            try:
                ...
                prompt_messages = [
                    {
                        "role": "system",
                        "content": """Ты — эксперт по созданию промптов для DALL-E 3.
Твоя задача: создать детальный технический промпт на АНГЛИЙСКОМ языке для генерации строительного/архитектурного изображения.

ТРЕБОВАНИЯ К ПРОМПТУ:
- Язык: ТОЛЬКО английский
- Стиль: Technical construction illustration, professional engineering drawing
- Детали: Точные размеры, материалы, конструктивные элементы
- Формат: Blueprint style, isometric view или technical drawing
- Качество: High detail, professional documentation quality

ВАЖНО - ТЕКСТОВОЕ ОПИСАНИЕ ДЛЯ TELEGRAM:
- После промпта для DALL-E добавь краткое описание на русском
- Описание должно быть коротким (до 200 слов)
- Формат для мобильного: макс 35 символов в строке
- НЕ используй широкие таблицы и ASCII-схемы
- Используй вертикальные списки с → ↓ •

Ответь в формате:
[English DALL-E prompt]

---ОПИСАНИЕ---
[Краткое описание на русском, адаптированное для телефона]"""
                    },
                    {
                        "role": "user",
                ...
                await generating_msg.edit_text(
                    "🎨 Генерирую изображение через DALL-E 3...\n"
                    "Шаг 2/2: DALL-E 3 создает изображение..."
                )

                # Шаг 2: Генерируем изображение через DALL-E с промптом от Grok
                result = await generate_construction_image_gemini(dalle_prompt)
