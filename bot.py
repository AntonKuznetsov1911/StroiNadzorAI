...
📱 ВАЖНО: ФОРМАТИРОВАНИЕ ДЛЯ МОБИЛЬНЫХ УСТРОЙСТВ:
• МАКСИМАЛЬНАЯ ШИРИНА: 30-35 символов в строке (пользователь на телефоне!)
• СХЕМЫ И ЧЕРТЕЖИ:
  — НЕ используйте широкие ASCII-таблицы (┌─┐│└┘)
  — ВМЕСТО схем используйте вертикальные списки с отступами и стрелками
  — Если нужна схема — делайте УЗКУЮ (не шире 30 символов)
  — Используйте простые символы: → ↓ • ◦ ▪
• ТАБЛИЦЫ: только вертикальные списки, НЕ горизонтальные таблицы
• ФОРМУЛЫ: на отдельных строках, компактно
• Пример хорошей схемы для телефона:

  Фундамент:
  ↓
  1. Подушка (песок 20см)
  2. Бетон М300 (40см)
     → арматура Ø12
  3. Гидроизоляция
  ↓
